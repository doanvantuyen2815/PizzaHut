...
BEGIN
	...
	SET @Total_Amount = 0
	IF ((SELECT COUNT(*) FROM dbo.OrderDetailTemp) > 0  )
		SET @Total_Amount += (SELECT SUM(Quantity * Price) FROM dbo.OrderDetailTemp , dbo.Items WHERE ID = IDItem )
	IF ((SELECT COUNT(*) FROM dbo.OrderDetailComboTemp) > 0 )
		SET @Total_Amount +=  (SELECT SUM(Quantity * Price) FROM dbo.OrderDetailComboTemp, dbo.Combo WHERE ID = IDCombo)
